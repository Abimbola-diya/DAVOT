...
import { Users, Package, Phone, MapPin, ChevronRight, ArrowLeft, Plus, X, Trash2, Tag } from 'lucide-react';
import { api } from '../api';
...
const formatBadgeCategory = (cat?: string | null) => {
  ...
  if (cat.toLowerCase() === 'direct consumer') return 'DIRECT';
  ...
  return (
    <div>
      <AnimatePresence mode="wait">
        {/* SCENARIO 1: INITIAL LANDING - CRISP MINIMALIST CARDS WITH BOLD HARD SHADOW & ANIMATIONS */}
        {selectedCategory === null ? (
          <motion.div
            key="categories-landing"
              ...
              {categories.map(cat => {
                ...
                return (
                  <motion.div
                    ...
                    whileTap={{ scale: 0.97, translateY: 1, boxShadow: '2px 2px 0px #0f172a' }}
          ...
          /* SCENARIO 2: CATEGORY SELECTED DRILL-DOWN VIEW WITH SMOOTH SLIDE MOTION */
          <motion.div
            key="category-drilldown"
            initial={{ opacity: 0, x: 16 }}
            animate={{ opacity: 1, x: 0 }}
            exit={{ opacity: 0, x: 16 }}
            transition={{ duration: 0.22, ease: "easeOut" }}
            style={{ padding: '8px 0 28px 0' }}
          >
            {/* Header & Back Button */}
                ...
                filteredCustomers.map(c => {
                  ...
                  return (
                    <motion.div 
                      ...
                      whileTap={{ scale: 0.985, translateY: 1, boxShadow: '2px 2px 0px #0f172a' }}
                      transition={{ duration: 0.12 }}
                      style={{ 
                        marginBottom: 0, 
                        padding: '16px', 
        ...
        {isAddCategoryOpen && (
          ...
          >
            <motion.div 
              ...
              style={{ maxWidth: '380px', padding: '24px', borderRadius: '20px', border: '2.5px solid #0f172a', boxShadow: '6px 6px 0px #0f172a' }}
            >
        ...
        {isAddCustomerOpen && (
          ...
          >
            <motion.div 
              ...
              transition={{ type: 'spring', damping: 25, stiffness: 350 }}
              onClick={e => e.stopPropagation()} 
              style={{ maxWidth: '400px', padding: '24px', borderRadius: '20px', border: '2.5px solid #0f172a', boxShadow: '6px 6px 0px #0f172a' }}
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                <h3 style={{ margin: 0, fontSize: '16px', color: '#0f172a', fontWeight: 800 }}>
                  Add New Customer
